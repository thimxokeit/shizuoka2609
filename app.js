...
/* ---------- 일정 행 HTML ---------- */
function stopHTML(s, i, day){
  return "<div class='stop' data-i='" + i + "'>" +
           "<div class='no' style='background:" + (CAT_COLOR[s.cat]||day.color) + "'>" + (i+1) + "</div>" +
           "<div class='bd'>" +
...
function renderPlan(){
  ...
  d.stops.forEach(function(s,i){ h += stopHTML(s,i,d); });
...
var mapDay=1, foodOn=false, mapReady=false, dayMarkers=[], selected=-1;
...
function drawDay(fit){
  ...
  L.polyline(pts, {color:d.color, weight:4, opacity:.85, dashArray:"1,9", lineCap:"round"})
   .addTo(dayLayer);
