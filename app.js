...
function buildDaybar(el, current, onPick){
  ...
  DAYS.forEach(function(d){
    ...
    b.innerHTML = "Day " + d.id + "<span class='dd'>" + esc(d.date) + "</span>";
...
function renderPlan(){
  ...
  var d = DAYS.filter(function(x){ return x.id === planDay; })[0];
  var h = "<div class='dayhead'><h2>Day " + d.id + " · " + esc(d.theme) + "</h2>" +
          "<div class='dt'>" + esc(d.date) + "</div></div>";
  d.stops.forEach(function(s, i){
    h += "<div class='stop'>" +
           "<div class='no' style='background:" + (CAT_COLOR[s.cat] || d.color) + "'>" + (i+1) + "</div>" +
           "<div class='bd'>" +
             "<div class='tm'>" + esc(s.t) + " · " + esc(s.cat) + "</div>" +
...
/* ================= 지도 ================= */
var map = null, tiles = null, dayLayer = null, foodLayer = null, meMarker = null;
var canvasRenderer = null;
var mapDay = 1, foodOn = false, mapReady = false;
...
function rebuildMapDaybar(){
  buildDaybar($('mapDaybar'), mapDay, function(id){
    mapDay = id; rebuildMapDaybar(); drawDay(true);
  });
}
...
function drawDay(fit){
  ...
  var d = DAYS.filter(function(x){ return x.id === mapDay; })[0];
  var pts = d.stops.map(function(s){ return [s.lat, s.lng]; });

  L.polyline(pts, {
    color: d.color, weight: 4, opacity: .85, dashArray: "1,9", lineCap: "round"
  }).addTo(dayLayer);
  ...
  d.stops.forEach(function(s, i){
    L.marker([s.lat, s.lng], {
      icon: L.divIcon({
        className: "",
        html: "<div class='num-pin' style='background:" + d.color + "'><span>" + (i+1) + "</span></div>",
        iconSize: [30,30], iconAnchor: [15,28], popupAnchor: [0,-26]
      })
    }).bindPopup(
      "<div class='pp-c' style='color:" + (CAT_COLOR[s.cat] || d.color) + "'>" + esc(s.t) + " · " + esc(s.cat) + "</div>" +
      "<div class='pp-n'>" + (i+1) + ". " + esc(s.name) + "</div>" +
      (s.note ? "<div class='pp-t'>" + esc(s.note) + "</div>" : "") +
      "<a class='pp-l' href='" + gmaps(s.lat, s.lng) + "' target='_blank' rel='noopener'>📍 길찾기</a>"
    ).addTo(dayLayer);
  });

  if (fit) map.fitBounds(L.latLngBounds(pts), { padding: [26,26] });
}
...
function drawFood(){
  ...
  FOOD.forEach(function(f){
    ...
    L.circleMarker([f.lat, f.lng], {
      ...
      radius: 7, weight: 2, color: "#fff", fillColor: col, fillOpacity: 1
    }).bindPopup(
...
function tileListForBounds(b, zooms){
  ...
  zooms.forEach(function(z){
    var x1 = lon2x(b[1], z), x2 = lon2x(b[3], z);
    var y1 = lat2y(b[2], z), y2 = lat2y(b[0], z);
...
/* 실제로 다니는 좁은 구역만. 대량 다운로드가 되지 않도록 상한을 둠 */
var SAVE_AREAS = [
  [34.9840,138.3720, 34.9640,138.3990],   // 시즈오카 시내 (도보권)
  [35.0250,138.4830, 34.9900,138.5280],   // 시미즈 · 미호노마츠바라
  [35.2320,138.6030, 35.2180,138.6220],   // 후지노미야 중심
  [35.1650,138.6700, 35.1480,138.7020]    // 후지시
];
var SAVE_ZOOMS = [13,14,15,16];
var SAVE_CAP = 900;
...
/* ================= 화면 전환 ================= */
var VIEWS = { plan:'view-plan', map:'view-map', food:'view-food' };
...
$('btnFit').addEventListener('click', function(){ drawDay(true); });

$('btnFood').addEventListener('click', function(){
  foodOn = !foodOn;
  this.setAttribute('aria-pressed', foodOn ? "true" : "false");
  this.textContent = foodOn ? "맛집 켜짐" : "맛집 표시";
  if (foodOn){ foodLayer.addTo(map); drawFood(); }
